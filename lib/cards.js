...
export function buildRecordsSvg(slug, data) {
  ...
  switch (slug) {
    ...
    case 'perfect-seasons': {
      ...
      if (!top) {
        return frame(title(`${SITE.losslessSeasonNoun.toUpperCase()} SEASONS`) + sub(168, `${SITE.fullName} · ${range}`)
          + line(360, 'None. Yet.', WHITE, 60));
      }
      return recordsFrame({
        heading: `${SITE.losslessSeasonNoun.toUpperCase()} SEASONS`, range,
    ...
    case 'best-seasons': {
      const [top, ...rest] = data.bestSeasons;
      return recordsFrame({
        heading: 'BEST SEASONS', range,
        big: top.record, context: `the ${top.season} season`,
        runners: [rest.map((b) => `${b.record} in ${b.season}`).join('  ·  ')],
      });
    }
    case 'worst-seasons': {
      const [top, ...rest] = data.worstSeasons;
      return recordsFrame({
        heading: 'WORST SEASONS', range,
        big: top.record, context: `the ${top.season} season`,
        runners: [rest.map((w) => `${w.record} in ${w.season}`).join('  ·  ')],
      });
    }
    case 'losing-streaks': {
      const [top, ...rest] = data.loseStreaks;
      if (!top) {
        return frame(title('LONGEST LOSING STREAKS') + sub(168, `${SITE.fullName} · ${range}`)
          + line(360, 'None. Remarkable.', WHITE, 60));
      }
      return recordsFrame({
        heading: 'LONGEST LOSING STREAKS', range,
    ...
    case 'playoff-appearances': {
      const all = data.playoffAppearances;
      if (!all.length) {
        return frame(title('PLAYOFF APPEARANCES') + sub(168, `${SITE.fullName} · ${range}`)
          + line(360, 'None. Yet.', WHITE, 60));
      }
      return recordsFrame({
        heading: 'PLAYOFF APPEARANCES', range,
        big: String(all.length), context: `most recently ${all[0].season}`,
        runners: [all.slice(0, 8).map((a) => String(a.season)).join('  ·  ')],
      });
    }
    case 'championship-appearances': {
      const all = data.championshipAppearances;
      const heading = `${SITE.championship.toUpperCase()} APPEARANCES`;
      if (!all.length) {
        return frame(title(heading) + sub(168, `${SITE.fullName} · ${range}`)
          + line(360, 'None. Yet.', WHITE, 60));
      }
      const won = all.filter((c) => c.won).length;
      return recordsFrame({
        heading, range,
        big: `${all.length}`, context: `${won} won`,
        runners: [all.map((c) => `${c.season} ${c.won ? 'won' : 'lost'}`).join('  ·  ')],
      });
    }
    case 'win-streaks': {
    ...
    case 'worst-losses': {
      ...
      const flag = (g) => (g.championship ? ', Super Bowl' : g.playoff ? ', playoffs' : '');
      return recordsFrame({
        heading: win ? 'MOST LOPSIDED WINS' : 'WORST LOSSES', range,
        big: `${top.pf}–${top.pa}`, bigColor: win ? GOLD : WHITE,
        context: `vs the ${top.opponent} · ${formatDate(top.date)}${top.championship ? ' · Super Bowl' : top.playoff ? ' · Playoffs' : ''}`,
    ...
    default: {
      ...
      const rows = [
        ...
        p
          ? `${SITE.losslessSeasonNoun} season — ${p.record} (${p.season})`
          : `${SITE.losslessSeasonNoun} season — none. Yet.`,
